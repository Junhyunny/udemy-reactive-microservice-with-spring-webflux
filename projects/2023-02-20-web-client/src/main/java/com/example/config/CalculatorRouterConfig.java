...
public class CalculatorRouterConfig {
    ...
    public RouterFunction<ServerResponse> serverResponseCalculatorRouterFunctions() {
        return RouterFunctions.route()
                ...
                .GET("{a}/{b}", isOperationExisted(), request -> ServerResponse.badRequest().bodyValue("OP should be +, -, *, /."))
                .build();
    }
    ...
    private RequestPredicate isOperationExisted() {
        return RequestPredicates.headers(
                headers -> headers.asHttpHeaders().toSingleValueMap().get("OP") != null
        );
    }
}
